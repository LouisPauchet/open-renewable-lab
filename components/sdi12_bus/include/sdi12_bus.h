 ...
 * UART. Matches Walter Feels' actual topology: separate TXD/RXD GPIOs
 * through two SN74LV1T126 tri-state buffers (BOARD_PIN_SDI12_TXD/RXD),
 * each gated by its own active-high enable pin
 ...
 * this board; sdi12_bus_init() holds those disabled (see board_pins.h
 * for the RS485/RS232 *_RX_EN active-low gotcha that caused a real bus
 * contention with SDI-12's own receive buffer on real hardware).
 * Confirmed end-to-end against a real sensor on real hardware.
 *
