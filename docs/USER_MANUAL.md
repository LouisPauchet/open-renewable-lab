...
| *(I2C sensors)* Device type ID | Which built-in driver to use to talk to this chip. `0` = ADS1113/1114/1115 analog-to-digital converter (returns a voltage). `1` = generic fallback that just reads a raw 16-bit register with no interpretation — use this if there's no dedicated driver yet for your sensor. |
| *(I2C sensors)* Channel index | For the ADC driver: which input channel (0-3). For the generic driver: which register address to read. |
